...
$currentYear = date('Y');
$dateCol = "IF(accStatus = 230, deliveryDate, progressDate)";

if ($monthFilter === 'current') {
    $whereClause .= " AND MONTH($dateCol) = MONTH(CURRENT_DATE()) AND YEAR($dateCol) = '$currentYear'";
} elseif (in_array($monthFilter, ['Q1', 'Q2', 'Q3', 'Q4'])) {
    if ($monthFilter === 'Q1') $whereClause .= " AND MONTH($dateCol) IN (1, 2, 3)";
    if ($monthFilter === 'Q2') $whereClause .= " AND MONTH($dateCol) IN (4, 5, 6)";
    if ($monthFilter === 'Q3') $whereClause .= " AND MONTH($dateCol) IN (7, 8, 9)";
    if ($monthFilter === 'Q4') $whereClause .= " AND MONTH($dateCol) IN (10, 11, 12)";
    $whereClause .= " AND YEAR($dateCol) = '$currentYear'";
    ...
    if (!empty($dateFrom) && !empty($dateTo)) {
        $whereClause .= " AND $dateCol BETWEEN '$dateFrom' AND '$dateTo'";
    } elseif (!empty($dateFrom)) {
        $whereClause .= " AND $dateCol >= '$dateFrom'";
    } elseif (!empty($dateTo)) {
        $whereClause .= " AND $dateCol <= '$dateTo'";
    }
} elseif ($monthFilter !== 'all' && preg_match('/^\d{2}$/', $monthFilter)) {
    $monthVal = intval($monthFilter);
    $whereClause .= " AND MONTH($dateCol) = $monthVal AND YEAR($dateCol) = '$currentYear'";
}
